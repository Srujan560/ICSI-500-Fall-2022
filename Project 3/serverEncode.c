...
int main(){
    ...
    if(fk==0){
        ...
        int wait2=0;
        if(!fork()){
            char* arr[]={"e","Pthread.tmp","ServerOutput.binary",NULL};
            execv("e",arr);
            fprintf(stderr,"Fail to Encode method From ServerEcode\n");
        }
        wait(&wait2);
        int wait3=0;
        printf("Just turn into binary file\n");
        if(!fork()){
            char* arr[]={"p","ServerOutput.binary",NULL};
            execv("p",arr);
            fprintf(stderr,"Fail to to run parity method From ServerEcode\n");
        }
        wait(&wait3);
        int wait4=0;
        printf("Just turn into parity bit \n");        
        if(!fork()){
            char* arr[]={"h","ServerOutput.binary","ServerHammingOutput.binary",NULL};
            execv("h",arr);
            fprintf(stderr,"Fail to to run parity method From ServerEcode\n");
        }
        wait(&wait4);  
